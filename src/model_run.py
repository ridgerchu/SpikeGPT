...
from src.spikingjelly.clock_driven import surrogate, neuron
...
class RWKV_RNN():  # this is running in FP32 at this moment
    ...
    def run(self, ctx):
        w = self.w
        x = self.atan(w.emb.weight[ctx[-1]])
        ...
        for i in range(self.n_layer):
            ...
            if i == 0 and self.model_type == 'RWKV-ffnPre':
                ...
                x = x + self.lif1(self.SA(self.LN(x, w.blocks[i].ln1), w.blocks[i].att, f'att.{i}'))
            x = x + self.lif2(self.FF(self.LN(x, w.blocks[i].ln2), w.blocks[i].ffn, f'ffn.{i}'))
